...
public class TestSheetOutput extends TalendFakeJob {
	...
	public static void testTypes() throws Exception {
		...
		// write header
		try {
			tFileExcelSheetOutput_1.writeRow(header_tFileExcelSheetOutput_1);
		} catch (Exception e) {
			globalMap.put("tFileExcelSheetOutput_1_ERROR_MESSAGE", "Error in header:" + e.getMessage());
			throw e;
		}
		...
		try {
			...
			globalMap.put("tFileExcelSheetOutput_1_ERROR_MESSAGE",
					"Error in line " + nb_line_tFileExcelSheetOutput_1 + ":" + e.getMessage());
			throw e;
		}
		...
		// persist workbook
		try {
			tFileExcelWorkbookSave_1.setOutputFile("/var/testdata/excel/excel_types_out.xlsx");
			...
			// release the memory
			globalMap.put("tFileExcelWorkbookSave_1_FILENAME", "/var/testdata/excel/excel_types_out.xlsx");
