...
public class SXSSFSheet implements Sheet, Cloneable
{
	...
	static {
		System.out.println("Load patched SXSSFSheet");
	}
...
//default to 10 on the first row.
    ...
    {
        if (row.getSheet() != this && row.getSheet() != _sh) {
    ...
    {
    	Row row = _rows.get(new Integer(rownum));
    	if (row == null) {
    ...
    {
    	int num = _rows.size()+_writer.getNumberOfFlushedRows();
    	if (num < _sh.getPhysicalNumberOfRows()) {
    		num = _sh.getPhysicalNumberOfRows();
    	}
    ...
    {
    	int firstRowNum = Integer.MAX_VALUE;
    	int firstRowNumInWriter;
        if (_writer.getNumberOfFlushedRows() > 0) {
        	firstRowNumInWriter = _writer.getLowestIndexOfFlushedRows();
        } else {
        	firstRowNumInWriter = Integer.MAX_VALUE;
        }
        int firstRowInternal = _rows.size() == 0 ? Integer.MAX_VALUE : _rows.firstKey();
        int firstRowXSSF = _sh.getLastRowNum() > 0 ? _sh.getFirstRowNum() : Integer.MAX_VALUE;
        if (firstRowNum > firstRowNumInWriter) {
        	firstRowNum = firstRowNumInWriter;
        }
        if (firstRowNum > firstRowInternal) {
        	firstRowNum = firstRowInternal;
        }
        if (firstRowNum > firstRowXSSF) {
        	firstRowNum = firstRowXSSF;
        }
        return firstRowNum;
    }
    ...
    {
    	int lastRowNum = _rows.size() == 0 ? 0 : _rows.lastKey();
    	if (lastRowNum < _sh.getLastRowNum()) {
    		lastRowNum = _sh.getLastRowNum();
    	}
    	return lastRowNum;
    }
